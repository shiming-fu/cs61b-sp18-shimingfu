public class KdTree {
    ...
    private GraphDB.Node nearest (GraphDB.Node p,KdTree.KdNode node, double min_Dist)

    {
        ...
        KdNode first = node.rightTop;
        KdNode second = node.leftBottom;
        if(node.compareX && p.lon <= node.point.lon || (!node.compareX && p.lat<=node.point.lat))
        {
